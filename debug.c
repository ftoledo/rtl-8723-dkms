...
static int rtl_proc_get_mac_0(struct seq_file *m, void *v)
{
	...
	for (n = 0; n <= max; ) {
		...
		for (i = 0; i < 4 && n <= max; i++, n += 4)
			seq_printf(m, "%8.8x    ", rtl_read_dword(rtlpriv, (page | n)));
	}
	seq_puts(m, "\n");
	return 0;
}
...
static int rtl_proc_get_reg_rf_b(struct seq_file *m, void *v)
{
	...
	for (n = 0; n <= max; ) {
		...
		for (i = 0; i < 4 && n <= max; n += 1, i++)
			seq_printf(m, "%8.8x    ",
				rtl_get_rfreg(hw, RF90_PATH_B, n, 0xffffffff));
	}
	seq_puts(m, "\n");
	return 0;
}
...
{
	return single_open(file, rtl_proc_get_cam_register_1, GET_INODE_DATA(inode));
} 
...
{
	return single_open(file, rtl_proc_get_cam_register_2, GET_INODE_DATA(inode));
} 
...
{
	return single_open(file, rtl_proc_get_cam_register_3, GET_INODE_DATA(inode));
} 
