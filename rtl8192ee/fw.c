...
static void _rtl92ee_fw_page_write(struct ieee80211_hw *hw,
...
static void _rtl92ee_write_fw(struct ieee80211_hw *hw,
			     enum version_8192e version, u8 *buffer, u32 size)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u8 *bufferPtr = (u8 *) buffer;
...
static void _rtl92ee_fill_h2c_command(struct ieee80211_hw *hw,
			      u8 element_id, u32 cmd_len, u8 *p_cmdbuffer)
{
	...
	/* 1. Prevent race condition in setting H2C cmd. (copy from MgntActSet_RF_State().) */
	while (true) {
		...
		if (isfw_read == true) {
			...
			while (!isfw_read) {
				...
				if (wait_h2c_limmit == 0) {
					RT_TRACE(COMP_CMD, DBG_LOUD,
						("FillH2CCommand8192E(): Wating too long "
						 "for FW read clear HMEBox(%d)!!!\n", boxnum));
					break;
				}
				udelay(10);
				isfw_read = _rtl92ee_check_fw_read_last_h2c(hw, boxnum);
...
void rtl92ee_set_fw_pwrmode_cmd(struct ieee80211_hw *hw, u8 mode)
{
	...
	rtl92ee_fill_h2c_cmd(hw, H2C_92E_SETPWRMODE, H2C_92E_PWEMODE_LENGTH, u1_h2c_set_pwrmode);
...
void rtl92ee_set_fw_joinbss_report_cmd(struct ieee80211_hw *hw, u8 mstatus)
{
	...
	rtl92ee_fill_h2c_cmd(hw, H2C_92E_JOINBSSRPT, 1, u1_joinbssrpt_parm);
}

void rtl92ee_set_fw_ap_off_load_cmd(struct ieee80211_hw *hw,  u8 ap_offload_enable)
{
	struct rtl_mac *mac = rtl_mac(rtl_priv(hw));
	u8 u1_apoffload_parm[H2C_92E_AP_OFFLOAD_LENGTH] = { 0 };

	SET_H2CCMD_AP_OFFLOAD_ON(u1_apoffload_parm, ap_offload_enable);
	SET_H2CCMD_AP_OFFLOAD_HIDDEN(u1_apoffload_parm, mac->bhiddenssid);
	SET_H2CCMD_AP_OFFLOAD_DENYANY(u1_apoffload_parm, 0);

	rtl92ee_fill_h2c_cmd(hw, H2C_92E_AP_OFFLOAD, H2C_92E_AP_OFFLOAD_LENGTH, u1_apoffload_parm);
...
void rtl92ee_set_p2p_ps_offload_cmd(struct ieee80211_hw *hw, u8 p2p_ps_state)
{
	...
	rtl92ee_fill_h2c_cmd(hw, H2C_92E_P2P_PS_OFFLOAD, 1, (u8 *)p2p_ps_offload);
