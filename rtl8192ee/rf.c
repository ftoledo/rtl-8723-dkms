...
void rtl92ee_phy_rf6052_set_cck_txpower(struct ieee80211_hw *hw,
				       u8 *ppowerlevel)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	...
	rtl92ee_dm_txpower_track_adjust(hw,1,&direction,&pwrtrac_value);
	if (direction ==1){
...
static void rtl92ee_phy_get_power_base(struct ieee80211_hw *hw,
...
static void _rtl92ee_get_txpower_writeval_by_regulatory(struct ieee80211_hw *hw,
						       u8 channel, u8 index,
						       u32 *powerBase0,
						       u32 *powerBase1,
						       u32 *p_outwriteval)
...
static void _rtl92ee_write_ofdm_power_reg(struct ieee80211_hw *hw,
					 u8 index, u32 *pValue)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u16 regoffset_a[6] = {
...
void rtl92ee_phy_rf6052_set_ofdm_txpower(struct ieee80211_hw *hw,
					      u8 *ppowerlevel_ofdm, u8 *ppowerlevel_bw20, u8 *ppowerlevel_bw40, u8 channel)
{
	...
	rtl92ee_phy_get_power_base(hw, ppowerlevel_ofdm, ppowerlevel_bw20, ppowerlevel_bw40,
				  channel, &powerBase0[0], &powerBase1[0]);

	rtl92ee_dm_txpower_track_adjust(hw,1,&direction,&pwrtrac_value);

	for (index = 0; index < 6; index++) {
		_rtl92ee_get_txpower_writeval_by_regulatory(hw,
							   channel, index,
							   &powerBase0[0],
							   &powerBase1[0],
							   &writeVal[0]);
		...
		_rtl92ee_write_ofdm_power_reg(hw, index, &writeVal[0]);
	}
}
