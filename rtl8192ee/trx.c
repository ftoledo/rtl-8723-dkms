...
static void _rtl92ee_query_rxphystatus(struct ieee80211_hw *hw,
			...
			struct rx_fwinfo_92e *p_drvinfo, bool bpacket_match_bssid,
			bool bpacket_toself, bool b_packet_beacon)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_ps_ctl *ppsc = rtl_psc(rtlpriv);
	struct phy_sts_cck_8192e_t *cck_buf;
	...
	if (b_is_cck) {
		...
		cck_buf = (struct phy_sts_cck_8192e_t *)p_drvinfo;
...
static void _rtl92ee_smart_antenna(struct ieee80211_hw *hw,
	struct rtl_stats *pstatus)
{
	...
	(rtlefuse->antenna_div_type == CGCS_RX_HW_ANTDIV)) {
		if (pstatus->b_packet_toself || pstatus->b_packet_matchbssid) {
			antsel_tr_mux = (pfat_table->antsel_rx_keep_2 << 2) |
					(pfat_table->antsel_rx_keep_1 << 1) | pfat_table->antsel_rx_keep_0;
			rtl92ee_dm_ant_sel_statistics(hw, antsel_tr_mux, 0, pstatus->rx_pwdb_all);
		}
...
static void _rtl92ee_translate_rx_signal_stuff(struct ieee80211_hw *hw,
		...
		u8 *pdesc, struct rx_fwinfo_92e *p_drvinfo)
{
	...
	_rtl92ee_query_rxphystatus(hw, pstatus, pdesc, p_drvinfo,
				   ...
				   b_packet_beacon);
	_rtl92ee_smart_antenna(hw, pstatus);
...
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rx_fwinfo_92e *p_drvinfo;
	struct ieee80211_hdr *hdr;
	...
	if (phystatus == true) {
		p_drvinfo = (struct rx_fwinfo_92e *)(skb->data +
						     status->rx_bufshift);

		_rtl92ee_translate_rx_signal_stuff(hw,
						   skb, status, pdesc,
						   p_drvinfo);
	}
...
/*<delete in kernel end>*/
	...
	CLEAR_PCI_TX_DESC_CONTENT(pdesc, sizeof(struct tx_desc_92e));
			...
			if (ptcb_desc->empkt_num) {
				RT_TRACE(COMP_SEND, DBG_TRACE,
					 ("Insert 8 byte.pTcb->EMPktNum:%d\n",
					  ptcb_desc->empkt_num));
				_rtl92ee_insert_emcontent(ptcb_desc, (u8 *)(skb->data));
			}
		} else {
...
/*<delete in kernel end>*/
	...
	rtl92ee_dm_set_tx_ant_by_tx_info(hw,pdesc,ptcb_desc->mac_id);
	RT_TRACE(COMP_SEND, DBG_TRACE, ("\n"));
}
...
void rtl92ee_set_desc(u8 *pdesc, bool istx, u8 desc_name, u8 *val)
{
	if (istx == true) {
		switch (desc_name) {
		case HW_DESC_OWN:
