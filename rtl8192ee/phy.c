			   ...
			   u32 regaddr, u32 bitmask, u32 data)
{
	...
	if (bitmask != RFREG_OFFSET_MASK) {
			original_value = _rtl92ee_phy_rf_serial_read(hw,
								    rfpath,
								    regaddr);
			bitshift = _rtl92ee_phy_calculate_bit_shift(bitmask);
			data =
	...
	_rtl92ee_phy_rf_serial_write(hw, rfpath, regaddr, data);
...
static bool _rtl92ee_check_condition(struct ieee80211_hw *hw,
	const u32  Condition
	)
{
...
static void _rtl92ee_config_rf_reg(struct ieee80211_hw *hw,
						u32 addr, u32 data,
						enum radio_path rfpath,
						u32 regaddr
	)
	...
	if (addr == 0xfe || addr == 0xffe) {
		...
		if (addr == 0xb2) {
			...
			while ( getvalue != data) {
				...
				udelay(1);
				rtl_set_rfreg(hw, rfpath, 0x18, RFREG_OFFSET_MASK, 0x0fc07);
				udelay(1);
				getvalue = rtl_get_rfreg(hw, rfpath, addr, MASKDWORD);
				if (count > 5)
					break;
	...
	u32 addr, u32 data)
{
	...
	_rtl92ee_config_rf_reg(hw, addr, data, RF90_PATH_A, addr | maskforphyset);
	...
	u32 addr, u32 data)
{
	...
	_rtl92ee_config_rf_reg(hw, addr, data, RF90_PATH_B, addr | maskforphyset);
...
u8 _rtl92ee_phy_get_txpower_by_rate_base(struct ieee80211_hw *hw,
					 u8 band, u8 path,
...
void _rtl92ee_phy_store_txpower_by_rate_base(struct ieee80211_hw *hw)
{
	...
	for (path = RF90_PATH_A; path <= RF90_PATH_B; ++path) {
		if (path == RF90_PATH_A) {
			...
			_rtl92ee_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, CCK, RF_1TX, base);
		...
		base = (rawValue >> 4) * 10 + (rawValue & 0xF);
		_rtl92ee_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, OFDM, RF_1TX, base );

		rawValue = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_1TX][5] >> 24) & 0xFF; 
		base = (rawValue >> 4) * 10 + (rawValue & 0xF);
		_rtl92ee_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, HT_MCS0_MCS7, RF_1TX, base );

		rawValue = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_2TX][7] >> 24) & 0xFF; 
		base = (rawValue >> 4) * 10 + (rawValue & 0xF);
		_rtl92ee_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, HT_MCS8_MCS15, RF_2TX, base );
	}	
}
...
void _rtl92ee_phy_convert_txpower_dbm_to_relative_value(struct ieee80211_hw *hw)
{
	...
	for (rfPath = RF90_PATH_A; rfPath <= RF90_PATH_B; ++rfPath) {
		if (rfPath == RF90_PATH_A) {
			base = _rtl92ee_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfPath, RF_1TX, CCK);
		...
		base = _rtl92ee_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfPath, RF_1TX, OFDM );
		_phy_convert_txpower_dbm_to_relative_value( 
		...
		base = _rtl92ee_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfPath, RF_1TX, HT_MCS0_MCS7 );
		_phy_convert_txpower_dbm_to_relative_value( 
		...
		base = _rtl92ee_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfPath, RF_2TX, HT_MCS8_MCS15 );
		_phy_convert_txpower_dbm_to_relative_value( 
						  ...
						  u8 configtype)
{
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		arraylen = RTL8192EE_PHY_REG_ARRAY_LEN;
		array_table = RTL8192EE_PHY_REG_ARRAY;
		...
		for (i = 0; i < arraylen; i = i + 2) {
			...
			if (v1 < 0xcdcdcdcd) {
				...
				if (!_rtl92ee_check_condition(hw,array_table[i])) {
			...
			arraylen = RTL8192EE_AGC_TAB_ARRAY_LEN;
			array_table = RTL8192EE_AGC_TAB_ARRAY;
						   ...
						   u32 bitmask, u32 data)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	u8 rate_section = _rtl92ee_get_rate_section_index(regaddr);
						    ...
						    u8 configtype)
{
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		for (i = 0; i < phy_regarray_pg_len; i = i + 6) {
			...
			if (v1<0xcdcdcdcd) {
				_rtl92ee_store_tx_power_by_rate(hw, v1, v2, v3, v4, v5, v6);
				continue;
			} else {
				if (!_rtl92ee_check_condition(hw, phy_regarray_table_pg[i]))
					  ...
					  enum radio_path rfpath)
{
	...
	switch (rfpath) {
	case RF90_PATH_A:
		radioa_arraylen = RTL8192EE_RADIOA_ARRAY_LEN;
		radioa_array_table = RTL8192EE_RADIOA_ARRAY;
		RT_TRACE(COMP_INIT, DBG_LOUD,
		...
		for (i = 0; i < radioa_arraylen; i = i + 2) {
			...
			if (v1 < 0xcdcdcdcd) {
				...
				if (!_rtl92ee_check_condition(hw,radioa_array_table[i])) {
					/*Discard the following (offset, data) pairs*/
	...
	case RF90_PATH_B:
		radioa_arraylen = RTL8192EE_RADIOB_ARRAY_LEN;
		radioa_array_table = RTL8192EE_RADIOB_ARRAY;
		RT_TRACE(COMP_INIT, DBG_LOUD,
...
void rtl92ee_phy_get_txpower_level(struct ieee80211_hw *hw, long *powerlevel)
{
	...
	if (_rtl92ee_phy_txpwr_idx_to_dbm(hw,
					 WIRELESS_MODE_G,
					 txpwr_level) > txpwr_dbm)
		txpwr_dbm =
		    _rtl92ee_phy_txpwr_idx_to_dbm(hw, WIRELESS_MODE_G,
						 txpwr_level);
	txpwr_level = rtlphy->cur_ofdm24g_txpwridx;
	if (_rtl92ee_phy_txpwr_idx_to_dbm(hw,
					 WIRELESS_MODE_N_24G,
					 txpwr_level) > txpwr_dbm)
		txpwr_dbm =
		    _rtl92ee_phy_txpwr_idx_to_dbm(hw, WIRELESS_MODE_N_24G,
						 txpwr_level);
	*powerlevel = txpwr_dbm;
}

static u8 _rtl92ee_phy_get_ratesection_intxpower_byrate(enum radio_path path, u8 rate)
{
	u8 rate_section = 0;
				...
				enum radio_path rfpath, u8 rate)
{
	...
	rate_section = _rtl92ee_phy_get_ratesection_intxpower_byrate(rfpath, rate);
	...
	switch (rate) {
	case DESC92C_RATE1M:	shift = 0;		break;
						...
						u8 bw, u8 channel)
{
	...
	/* OFDM-1T*/
	if (DESC92C_RATE6M <= rate && rate <= DESC92C_RATE54M && !IS_CCK_RATE(rate))
	...
	if (rtlefuse->eeprom_regulatory != 2)
		power_diff_byrate = _rtl92ee_get_txpower_by_rate(hw, BAND_ON_2_4G, rfpath, rate);
		
	tx_power += power_diff_byrate;
...
/*
static void _rtl92ee_ccxpower_index_check(struct ieee80211_hw *hw,
...
void _rtl92ee_set_txpower_index(struct ieee80211_hw *hw,
								u8 power_index,
								enum radio_path rfpath,
								u8 rate)
{
	...
	if (rfpath == RF90_PATH_A) {
        switch (rate) {
        case DESC92C_RATE1M:    rtl_set_bbreg(hw, RTXAGC_A_CCK1_MCS32, MASKBYTE1, power_index); break;
        case DESC92C_RATE2M:    rtl_set_bbreg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE1, power_index); break;
        case DESC92C_RATE5_5M:  rtl_set_bbreg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE2, power_index); break;
        case DESC92C_RATE11M:   rtl_set_bbreg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE3, power_index); break;

        case DESC92C_RATE6M:    rtl_set_bbreg(hw, RTXAGC_A_RATE18_06, MASKBYTE0, power_index); break;
        case DESC92C_RATE9M:    rtl_set_bbreg(hw, RTXAGC_A_RATE18_06, MASKBYTE1, power_index); break;
        case DESC92C_RATE12M:   rtl_set_bbreg(hw, RTXAGC_A_RATE18_06, MASKBYTE2, power_index); break;
        case DESC92C_RATE18M:   rtl_set_bbreg(hw, RTXAGC_A_RATE18_06, MASKBYTE3, power_index); break;

        case DESC92C_RATE24M:   rtl_set_bbreg(hw, RTXAGC_A_RATE54_24, MASKBYTE0, power_index); break;
        case DESC92C_RATE36M:   rtl_set_bbreg(hw, RTXAGC_A_RATE54_24, MASKBYTE1, power_index); break;
        case DESC92C_RATE48M:   rtl_set_bbreg(hw, RTXAGC_A_RATE54_24, MASKBYTE2, power_index); break;
        case DESC92C_RATE54M:   rtl_set_bbreg(hw, RTXAGC_A_RATE54_24, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS0:  rtl_set_bbreg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS1:  rtl_set_bbreg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS2:  rtl_set_bbreg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS3:  rtl_set_bbreg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS4:  rtl_set_bbreg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS5:  rtl_set_bbreg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS6:  rtl_set_bbreg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS7:  rtl_set_bbreg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS8:  rtl_set_bbreg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS9:  rtl_set_bbreg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS10: rtl_set_bbreg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS11: rtl_set_bbreg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS12: rtl_set_bbreg(hw, RTXAGC_A_MCS15_MCS12, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS13: rtl_set_bbreg(hw, RTXAGC_A_MCS15_MCS12, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS14: rtl_set_bbreg(hw, RTXAGC_A_MCS15_MCS12, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS15: rtl_set_bbreg(hw, RTXAGC_A_MCS15_MCS12, MASKBYTE3, power_index); break;
        ...
        switch (rate) {
        case DESC92C_RATE1M:    rtl_set_bbreg(hw, RTXAGC_B_CCK1_55_MCS32, MASKBYTE1, power_index); break;
        case DESC92C_RATE2M:    rtl_set_bbreg(hw, RTXAGC_B_CCK1_55_MCS32, MASKBYTE2, power_index); break;
        case DESC92C_RATE5_5M:  rtl_set_bbreg(hw, RTXAGC_B_CCK1_55_MCS32, MASKBYTE3, power_index); break;
        case DESC92C_RATE11M:   rtl_set_bbreg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE0, power_index); break;

        case DESC92C_RATE6M:    rtl_set_bbreg(hw, RTXAGC_B_RATE18_06, MASKBYTE0, power_index); break;
        case DESC92C_RATE9M:    rtl_set_bbreg(hw, RTXAGC_B_RATE18_06, MASKBYTE1, power_index); break;
        case DESC92C_RATE12M:   rtl_set_bbreg(hw, RTXAGC_B_RATE18_06, MASKBYTE2, power_index); break;
        case DESC92C_RATE18M:   rtl_set_bbreg(hw, RTXAGC_B_RATE18_06, MASKBYTE3, power_index); break;

        case DESC92C_RATE24M:   rtl_set_bbreg(hw, RTXAGC_B_RATE54_24, MASKBYTE0, power_index); break;
        case DESC92C_RATE36M:   rtl_set_bbreg(hw, RTXAGC_B_RATE54_24, MASKBYTE1, power_index); break;
        case DESC92C_RATE48M:   rtl_set_bbreg(hw, RTXAGC_B_RATE54_24, MASKBYTE2, power_index); break;
        case DESC92C_RATE54M:   rtl_set_bbreg(hw, RTXAGC_B_RATE54_24, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS0:  rtl_set_bbreg(hw, RTXAGC_B_MCS03_MCS00, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS1:  rtl_set_bbreg(hw, RTXAGC_B_MCS03_MCS00, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS2:  rtl_set_bbreg(hw, RTXAGC_B_MCS03_MCS00, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS3:  rtl_set_bbreg(hw, RTXAGC_B_MCS03_MCS00, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS4:  rtl_set_bbreg(hw, RTXAGC_B_MCS07_MCS04, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS5:  rtl_set_bbreg(hw, RTXAGC_B_MCS07_MCS04, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS6:  rtl_set_bbreg(hw, RTXAGC_B_MCS07_MCS04, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS7:  rtl_set_bbreg(hw, RTXAGC_B_MCS07_MCS04, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS8:  rtl_set_bbreg(hw, RTXAGC_B_MCS11_MCS08, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS9:  rtl_set_bbreg(hw, RTXAGC_B_MCS11_MCS08, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS10: rtl_set_bbreg(hw, RTXAGC_B_MCS11_MCS08, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS11: rtl_set_bbreg(hw, RTXAGC_B_MCS11_MCS08, MASKBYTE3, power_index); break;

        case DESC92C_RATEMCS12: rtl_set_bbreg(hw, RTXAGC_B_MCS15_MCS12, MASKBYTE0, power_index); break;
        case DESC92C_RATEMCS13: rtl_set_bbreg(hw, RTXAGC_B_MCS15_MCS12, MASKBYTE1, power_index); break;
        case DESC92C_RATEMCS14: rtl_set_bbreg(hw, RTXAGC_B_MCS15_MCS12, MASKBYTE2, power_index); break;
        case DESC92C_RATEMCS15: rtl_set_bbreg(hw, RTXAGC_B_MCS15_MCS12, MASKBYTE3, power_index); break;
											...
											u8 channel, u8 *rates, u8 size)
{
	...
	u8 power_index;
	for (i = 0; i < size; i++) {
		power_index = _rtl92ee_get_txpower_index(hw, rfpath, rates[i], bw, channel);
									...
									enum rate_section ratesection)
{
	...
	if (ratesection == CCK) {
		u8 cck_rates[] = {DESC92C_RATE1M, DESC92C_RATE2M, DESC92C_RATE5_5M, DESC92C_RATE11M};
		if (rtlhal->current_bandtype == BAND_ON_2_4G)
			rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
											rtlphy->current_chan_bw,
											channel, cck_rates, 4);
	} else if (ratesection == OFDM) {
		u8 ofdm_rates[] = {DESC92C_RATE6M, DESC92C_RATE9M, DESC92C_RATE12M,
						   DESC92C_RATE18M, DESC92C_RATE24M, DESC92C_RATE36M,
						   DESC92C_RATE48M, DESC92C_RATE54M};
		rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
											rtlphy->current_chan_bw,
											channel, ofdm_rates, 8);
	} else if (ratesection == HT_MCS0_MCS7) {
		u8 ht_rates1t[]  = {DESC92C_RATEMCS0, DESC92C_RATEMCS1, DESC92C_RATEMCS2,
							DESC92C_RATEMCS3, DESC92C_RATEMCS4, DESC92C_RATEMCS5,
							DESC92C_RATEMCS6, DESC92C_RATEMCS7};
		rtl92ee_phy_set_txpower_index_by_rate_array(hw, rfpath,
											rtlphy->current_chan_bw,
											channel, ht_rates1t, 8);
	} else if (ratesection == HT_MCS8_MCS15) {
		u8 ht_rates2t[]  = {DESC92C_RATEMCS8, DESC92C_RATEMCS9, DESC92C_RATEMCS10,
							DESC92C_RATEMCS11, DESC92C_RATEMCS12, DESC92C_RATEMCS13,
...
void rtl92ee_phy_set_txpower_level(struct ieee80211_hw *hw, u8 channel)
{
	...
	for (rfpath = RF90_PATH_A; rfpath < rtlphy->num_total_rfpath; rfpath++) {
		rtl92ee_phy_set_txpower_index_by_rate_section(hw, rfpath, channel, CCK );
		rtl92ee_phy_set_txpower_index_by_rate_section(hw, rfpath, channel, OFDM );
		rtl92ee_phy_set_txpower_index_by_rate_section(hw, rfpath, channel, HT_MCS0_MCS7 );
		
		if (rtlphy->num_total_rfpath >= 2)
			rtl92ee_phy_set_txpower_index_by_rate_section(hw, rfpath, channel, HT_MCS8_MCS15);
	}
...
void rtl92ee_phy_set_bw_mode_callback(struct ieee80211_hw *hw)
{
	...
	case HT_CHANNEL_WIDTH_20:
		rtl_set_bbreg(hw, RFPGA0_RFMOD, BIT(0), 0x0); 
		rtl_set_bbreg(hw, RFPGA1_RFMOD, BIT(0), 0x0); 
		rtl_set_rfreg(hw, RF90_PATH_A, RF_CHNLBW, BIT(11) | BIT(10), 0x3);			
		rtl_set_rfreg(hw, RF90_PATH_B, RF_CHNLBW, BIT(11) | BIT(10), 0x3);	
		break;
	case HT_CHANNEL_WIDTH_20_40:
		rtl_set_bbreg(hw, RFPGA0_RFMOD, BIT(0), 0x1); 
		rtl_set_bbreg(hw, RFPGA1_RFMOD, BIT(0), 0x1); 
		rtl_set_rfreg(hw, RF90_PATH_A, RF_CHNLBW, BIT(11) | BIT(10), 0x1);			
		rtl_set_rfreg(hw, RF90_PATH_B, RF_CHNLBW, BIT(11) | BIT(10), 0x1);	
		break;
	default:
		RT_TRACE(COMP_ERR, DBG_EMERG,
			 ("unknown bandwidth: %#X\n", rtlphy->current_chan_bw));
...
static u8 _rtl92ee_phy_path_b_rx_iqk(struct ieee80211_hw *hw, bool config_pathb)
{
	...
	u32temp = 0x80007C00 | (reg_eb4 & 0x3FF0000)  | ((reg_ebc & 0x3FF0000) >> 16);
	rtl_set_bbreg(hw, RTx_IQK, MASKDWORD, u32temp);
	/*RX IQK*/
	/*Modify RX IQK mode table*/
	rtl_set_bbreg(hw, RFPGA0_IQK, MASKDWORD, 0x00000000);
...
static void _rtl92ee_phy_path_adda_on(struct ieee80211_hw *hw,
...
/*
static void _rtl92ee_phy_pi_mode_switch(struct ieee80211_hw *hw, bool pi_mode)
{
	u32 mode;
				     ...
				     long result[][8], u8 t, bool is2t)
{
	...
	for(i = 0 ; i < retrycount ; i++) {
		...
		if (patha_ok == 0x01) {
			...
			result[t][0] = (rtl_get_bbreg(hw, RTx_Power_Before_IQK_A, MASKDWORD) & 0x3FF0000) >> 16;
			result[t][1] = (rtl_get_bbreg(hw, RTx_Power_After_IQK_A, MASKDWORD) & 0x3FF0000) >> 16;
			break;
		} else {
			RT_TRACE(COMP_RF, DBG_LOUD, ("Path A Tx IQK Fail!!, ret = 0x%x\n",patha_ok));
		}	
	}
	...
	for(i = 0 ; i < retrycount ; i++){
		...
		if (patha_ok == 0x03) {
			RT_TRACE(COMP_RF, DBG_LOUD, ("Path A Rx IQK Success!!\n"));
			result[t][2] = (rtl_get_bbreg(hw, RRx_Power_Before_IQK_A_2, MASKDWORD) & 0x3FF0000) >> 16;
			result[t][3] = (rtl_get_bbreg(hw, RRx_Power_After_IQK_A_2, MASKDWORD) & 0x3FF0000) >> 16;
			break;
		} else {
			RT_TRACE(COMP_RF, DBG_LOUD, ("Path A Rx IQK Fail!!, ret = 0x%x\n", patha_ok));		
		}
	}
	...
	if (is2t) {
		...
		for (i = 0 ; i < retrycount ; i++) {
			...
			if (pathb_ok == 0x01) {
				RT_TRACE(COMP_RF, DBG_LOUD, ("Path B Tx IQK Success!!\n"));
				result[t][4] = (rtl_get_bbreg(hw, RTx_Power_Before_IQK_B, MASKDWORD) & 0x3FF0000) >> 16;
				result[t][5] = (rtl_get_bbreg(hw, RTx_Power_After_IQK_B, MASKDWORD) & 0x3FF0000) >> 16;
				break;
			} else {
				RT_TRACE(COMP_RF, DBG_LOUD, ("Path B Tx IQK Fail!!, ret = 0x%x\n",pathb_ok));		
			}
		}
		...
		for (i = 0 ; i < retrycount ; i++) {
			...
			if (pathb_ok == 0x03) {
				RT_TRACE(COMP_RF, DBG_LOUD, ("Path B Rx IQK Success!!\n"));
				result[t][6] = (rtl_get_bbreg(hw, RRx_Power_Before_IQK_B_2, MASKDWORD) & 0x3FF0000) >> 16;
				result[t][7] = (rtl_get_bbreg(hw, RRx_Power_After_IQK_B_2, MASKDWORD) & 0x3FF0000) >> 16;
				break;
			} else {
				RT_TRACE(COMP_RF, DBG_LOUD, ("Path B Rx IQK Fail!!, ret = 0x%x\n",pathb_ok));		
			}
		}
	
		if (0x00 == pathb_ok)	
			RT_TRACE(COMP_RF, DBG_LOUD, ("Path B IQK failed!!, ret = 0\n"));		
	}
	//Back to BB mode, load original value
	RT_TRACE(COMP_RF, DBG_LOUD, ("IQK:Back to BB mode, load original value!\n"));
	rtl_set_bbreg(hw, RFPGA0_IQK, MASKDWORD, 0);

	if (t != 0) {
	 	/* Reload ADDA power saving parameters */
	 	_rtl92ee_phy_reload_adda_registers(hw, adda_reg, rtlphy->adda_backup, IQK_ADDA_REG_NUM);

		/* Reload MAC parameters */
		_rtl92ee_phy_reload_mac_registers(hw, iqk_mac_reg, rtlphy->iqk_mac_backup);
		
	 	_rtl92ee_phy_reload_adda_registers(hw, iqk_bb_reg, rtlphy->iqk_bb_backup, IQK_BB_REG_NUM);
...
void rtl92ee_phy_iq_calibrate(struct ieee80211_hw *hw, bool b_recovery)
{
	...
	_rtl92ee_phy_path_b_fill_iqk_matrix(hw, b_pathb_ok, result,
						final_candidate, (reg_ec4 == 0));

	index_for_channel = rtl92ee_get_rightchnlplace_for_iqk(rtlphy->current_channel);
	
	/*To Fix BSOD when final_candidate is 0xff
	  *by sherry 20120321*/
	if (final_candidate < 4) {
		for(i = 0; i < IQK_MATRIX_REG_NUM; i++)
			rtlphy->iqk_matrix_regsetting[index_for_channel].value[0][i] =
													result[final_candidate][i];
		
		rtlphy->iqk_matrix_regsetting[index_for_channel].b_iqk_done= true;		
	}
	_rtl92ee_phy_save_adda_registers(hw, iqk_bb_reg,
					rtlphy->iqk_bb_backup, 9);
}
...
static void rtl92ee_phy_set_io(struct ieee80211_hw *hw)
{
	...
	case IO_CMD_RESUME_DM_BY_SCAN:
		...
		rtl92ee_dm_write_dig(hw);
		dm_digtable.cur_cck_cca_thres = rtlphy->initgain_backup.cca;
		rtl92ee_dm_write_cck_cca_thres(hw);
	...
	case IO_CMD_PAUSE_DM_BY_SCAN:
		...
		dm_digtable.cur_cck_cca_thres = 0x40;
		rtl92ee_dm_write_cck_cca_thres(hw);
		break;
	default:
		RT_TRACE(COMP_ERR, DBG_EMERG,
