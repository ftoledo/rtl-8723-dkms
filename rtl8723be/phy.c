...
static bool _rtl8723be_phy_config_bb_with_pgheaderfile(struct ieee80211_hw *hw,
						    u8 configtype);
static void _rtl8723be_phy_init_bb_rf_register_definition(struct ieee80211_hw *hw);
...
u32 rtl8723be_phy_query_bb_reg(struct ieee80211_hw *hw, u32 regaddr, u32 bitmask)
{
...
void rtl8723be_phy_set_bb_reg(struct ieee80211_hw *hw,
			   ...
			   u32 regaddr, u32 bitmask, u32 data)
{
	...
	if (bitmask != RFREG_OFFSET_MASK) {
			original_value = _rtl8723be_phy_rf_serial_read(hw,
								    rfpath,
								    regaddr);
			bitshift = _rtl8723be_phy_calculate_bit_shift(bitmask);
			data =
	...
	_rtl8723be_phy_rf_serial_write(hw, rfpath, regaddr, data);
...
static void _rtl8723be_config_rf_reg(struct ieee80211_hw *hw,
	u32 addr,
	u32 data,
	enum radio_path rfpath,
	u32 regaddr
	...
	u32 addr, u32 data)
{
	...
	_rtl8723be_config_rf_reg(hw, addr, data, RF90_PATH_A, addr | maskforphyset);

}

#if 0
static void _rtl8723be_config_rf_radio_b(struct ieee80211_hw *hw,
	u32 addr, u32 data)
{
	u32 content = 0x1001; /*RF Content: radio_b_txt*/
	u32 maskforphyset = (u32)(content & 0xE000);

	_rtl8723be_config_rf_reg(hw, addr, data, RF90_PATH_B, addr | maskforphyset);
...
void _rtl8723be_phy_set_txpower_by_rate_base(struct ieee80211_hw *hw,
					   u8 band, u8 path,
					   u8 rate_section,
					   u8 txnum, u8 value)
{
...
u8 _rtl8723be_phy_get_txpower_by_rate_base(struct ieee80211_hw *hw,
					 u8 band, u8 path,
...
void _rtl8723be_phy_store_txpower_by_rate_base(struct ieee80211_hw *hw)
{
	...
	for (path = RF90_PATH_A; path <= RF90_PATH_B; ++path) {
		if (path == RF90_PATH_A) {
			...
			_rtl8723be_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, CCK, RF_1TX, base);
		...
		base = (rawValue >> 4) * 10 + (rawValue & 0xF);
		_rtl8723be_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, OFDM, RF_1TX, base );

		rawValue = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_1TX][5] >> 24) & 0xFF; 
		base = (rawValue >> 4) * 10 + (rawValue & 0xF);
		_rtl8723be_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, HT_MCS0_MCS7, RF_1TX, base );

		rawValue = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_2TX][7] >> 24) & 0xFF; 
		base = (rawValue >> 4) * 10 + (rawValue & 0xF);
		_rtl8723be_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, HT_MCS8_MCS15, RF_2TX, base );
	}	
}
...
void _rtl8723be_phy_convert_txpower_dbm_to_relative_value(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	u8 base = 0, rfPath = RF90_PATH_A;

	base = _rtl8723be_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfPath, RF_1TX, CCK);
	...
	base = _rtl8723be_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfPath, RF_1TX, OFDM );
	_phy_convert_txpower_dbm_to_relative_value( 
...
static void _rtl8723be_store_tx_power_by_rate(struct ieee80211_hw *hw,
						   u32 band, u32 rfpath, u32 txnum, u32 regaddr, u32 bitmask,
						   u32 data)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
						    ...
						    u8 configtype)
{
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		for (i = 0; i < phy_regarray_pg_len; i = i + 6) {
			...
			if(v1<0xcdcdcdcd){
				if (phy_regarray_table_pg[i] == 0xfe || phy_regarray_table_pg[i] == 0xffe)
					mdelay(50);
				else
				...
				continue;
			        ...
			        if (!_rtl8723be_check_condition(hw,phy_regarray_table_pg[i]))
	...
	case RF90_PATH_A:
		for (i = 0; i < radioa_arraylen; i = i + 2) {
			...
			if(v1<0xcdcdcdcd){
				...
				if(!_rtl8723be_check_condition(hw,radioa_array_table[i])){
					/*Discard the following (offset, data) pairs*/
...
void rtl8723be_phy_get_txpower_level(struct ieee80211_hw *hw, long *powerlevel)
{
	...
	txpwr_dbm = _rtl8723be_phy_txpwr_idx_to_dbm(hw,
						 WIRELESS_MODE_B, txpwr_level);
	txpwr_level = rtlphy->cur_ofdm24g_txpwridx;
	if (_rtl8723be_phy_txpwr_idx_to_dbm(hw,
					 WIRELESS_MODE_G,
				...
				enum radio_path rfpath, u8 rate)
{
	...
	rate_section = _rtl8723be_phy_get_ratesection_intxpower_byrate(rfpath, rate);
				      ...
				      u8 rate, u8 bandwidth,u8 channel)
{
	...
	if (DESC92C_RATE6M <= rate && rate <= DESC92C_RATE54M && !RX_HAL_IS_CCK_RATE(rate))
	...
	if (rtlefuse->eeprom_regulatory != 2)
		power_diff_byrate = _rtl8723be_get_txpower_by_rate(hw, BAND_ON_2_4G, path, rate);
...
static void _rtl8723be_phy_set_txpower_index(struct ieee80211_hw *hw ,u8 power_index, u8 path, u8 rate)
{
	struct rtl_priv* rtlpriv = rtl_priv(hw);
	if (path == RF90_PATH_A)
    	{
        switch (rate)
       {
            case DESC92C_RATE1M:    rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_CCK1_MCS32, MASKBYTE1, power_index); break;
            case DESC92C_RATE2M:    rtl8723be_phy_set_bb_reg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE1, power_index); break;
            case DESC92C_RATE5_5M:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE2, power_index); break;
            case DESC92C_RATE11M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_B_CCK11_A_CCK2_11, MASKBYTE3, power_index); break;

            case DESC92C_RATE6M:    rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE18_06, MASKBYTE0, power_index); break;
            case DESC92C_RATE9M:    rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE18_06, MASKBYTE1, power_index); break;
            case DESC92C_RATE12M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE18_06, MASKBYTE2, power_index); break;
            case DESC92C_RATE18M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE18_06, MASKBYTE3, power_index); break;

            case DESC92C_RATE24M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE54_24, MASKBYTE0, power_index); break;
            case DESC92C_RATE36M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE54_24, MASKBYTE1, power_index); break;
            case DESC92C_RATE48M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE54_24, MASKBYTE2, power_index); break;
            case DESC92C_RATE54M:   rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_RATE54_24, MASKBYTE3, power_index); break;

            case DESC92C_RATEMCS0:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE0, power_index); break;
            case DESC92C_RATEMCS1:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE1, power_index); break;
            case DESC92C_RATEMCS2:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE2, power_index); break;
            case DESC92C_RATEMCS3:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS03_MCS00, MASKBYTE3, power_index); break;

            case DESC92C_RATEMCS4:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE0, power_index); break;
            case DESC92C_RATEMCS5:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE1, power_index); break;
            case DESC92C_RATEMCS6:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE2, power_index); break;
            case DESC92C_RATEMCS7:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS07_MCS04, MASKBYTE3, power_index); break;

            case DESC92C_RATEMCS8:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE0, power_index); break;
            case DESC92C_RATEMCS9:  rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE1, power_index); break;
            case DESC92C_RATEMCS10: rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE2, power_index); break;
            case DESC92C_RATEMCS11: rtl8723be_phy_set_bb_reg(hw, RTXAGC_A_MCS11_MCS08, MASKBYTE3, power_index); break;
...
void rtl8723be_phy_set_txpower_level(struct ieee80211_hw *hw, u8 channel)
{
	...
	u8 ofdm_rates[]  = {DESC92C_RATE6M, DESC92C_RATE9M, DESC92C_RATE12M, DESC92C_RATE18M, DESC92C_RATE24M, DESC92C_RATE36M, DESC92C_RATE48M, DESC92C_RATE54M};
	u8 ht_rates_1t[]  = {DESC92C_RATEMCS0, DESC92C_RATEMCS1, DESC92C_RATEMCS2, DESC92C_RATEMCS3, DESC92C_RATEMCS4, DESC92C_RATEMCS5, DESC92C_RATEMCS6, DESC92C_RATEMCS7};
	u8 i,size;
	u8 power_index;
	...
	for (i=0; i<size; i++)
	{
		power_index = _rtl8723be_get_txpower_index(hw,RF90_PATH_A,cck_rates[i],rtl_priv(hw)->phy.current_chan_bw,channel);
		_rtl8723be_phy_set_txpower_index(hw,power_index,RF90_PATH_A,cck_rates[i]);
	}
	size = sizeof(ofdm_rates)/sizeof(u8);
	for (i=0; i<size; i++)
	{
		power_index = _rtl8723be_get_txpower_index(hw,RF90_PATH_A,ofdm_rates[i],rtl_priv(hw)->phy.current_chan_bw,channel);
		_rtl8723be_phy_set_txpower_index(hw,power_index,RF90_PATH_A,ofdm_rates[i]);
	}
	size = sizeof(ht_rates_1t)/sizeof(u8);
	for (i=0; i<size; i++)
	{
		power_index = _rtl8723be_get_txpower_index(hw,RF90_PATH_A,ht_rates_1t[i],rtl_priv(hw)->phy.current_chan_bw,channel);
		_rtl8723be_phy_set_txpower_index(hw,power_index,RF90_PATH_A,ht_rates_1t[i]);
	}
}
...
void rtl8723be_phy_sw_chnl_callback(struct ieee80211_hw *hw)
{
	...
	do {
		if (!rtlphy->sw_chnl_inprogress)
			break;
		if (!_rtl8723be_phy_sw_chnl_step_by_step
...
static u8 _rtl8723be_phy_path_a_rx_iqk(struct ieee80211_hw *hw, bool config_pathb)
{
...
static void _rtl8723be_phy_path_adda_on(struct ieee80211_hw *hw,
...
#if 0
	for (i = 0; i < retrycount; i++) {
		patha_ok = _rtl8723be_phy_path_a_rx_iqk(hw, is2t);
		if (patha_ok == 0x03) {
...
void rtl8723be_phy_iq_calibrate(struct ieee80211_hw *hw, bool b_recovery)
{
	...
	if (b_recovery) {
		_rtl8723be_phy_reload_adda_registers(hw,
						  iqk_bb_reg,
