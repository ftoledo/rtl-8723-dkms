...
static void rtl8723be_dm_init_txpower_tracking(struct ieee80211_hw *hw)
{
	...
	rtlpriv->dm.bb_swing_idx_ofdm_base = rtlpriv->dm.ofdm_index[0];
	rtlpriv->dm.delta_power_index = 0;
	rtlpriv->dm.delta_power_index_last = 0;
	rtlpriv->dm.power_index_offset = 0;
							     ...
							     *hw)
{
	...
	if (delta > 0 && rtlpriv->dm.txpower_track_control) {
		...
		if (thermalvalue > rtlefuse->eeprom_thermalmeter) {
			rtldm->delta_power_index_last = rtldm->delta_power_index;
			rtldm->delta_power_index = delta_swing_table_idx_tup_a[delta];
		} else {
			rtldm->delta_power_index_last = rtldm->delta_power_index;
			rtldm->delta_power_index = -1 * delta_swing_table_idx_tdown_a[delta];
		}

		/* 7.2 Handle boundary conditions of index.*/
		if (rtldm->delta_power_index == rtldm->delta_power_index_last)
			rtldm->power_index_offset = 0;
		else
			rtldm->power_index_offset = rtldm->delta_power_index - rtldm->delta_power_index_last;

		rtldm->ofdm_index[0] = rtldm->bb_swing_idx_ofdm_base + rtldm->power_index_offset;
		rtldm->cck_index = rtldm->bb_swing_idx_cck_base + rtldm->power_index_offset;
	...
	if ((rtldm->power_index_offset != 0) && 
		...
		rtldm->bb_swing_idx_ofdm_base = rtldm->bb_swing_idx_ofdm[0];
